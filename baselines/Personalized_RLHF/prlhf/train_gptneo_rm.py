...
# ----------------------------------------------------------------------
# GPT-Neo classes from transformers
# ----------------------------------------------------------------------
from transformers.models.gpt_neo import GPTNeoPreTrainedModel, GPTNeoModel

# ----------------------------------------------------------------------
# Local modules (you must have these in your codebase)
# If you don't, remove or adapt references:
# ----------------------------------------------------------------------
from utils import (
...
class ScriptArguments:
    """
    The arguments for GPT-Neo reward model training script.
    """
    # Data & subset
    ...
    # GPT-Neo model & tokenizer
    model_name: Optional[str] = field(
        default="EleutherAI/gpt-neo-125M", 
        metadata={"help": "Name/path of the GPT-Neo model (e.g. EleutherAI/gpt-neo-1.3B)."}
    )
    model_class: Optional[str] = field(
        default="gptneo",
        metadata={"help": "Model class to use. Must be 'gptneo' to use UserGPTNeoForRewardModel."}
    )
    ...
    initialize_from_vocab: Optional[bool] = field(default=True, metadata={"help": "Init user embeddings from GPT-Neo's vocab."})
    ...
    beta: Optional[float] = field(default=0.5, metadata={"help": "Beta param for DPO-like usage (not strictly needed for reward model)."})
...
# ----------------------------------------------------------------------
# Mixin for GPT-Neo + user embeddings
# ----------------------------------------------------------------------
class UserGPTNeoMixin:
    """
    Mixin that adds user-embedding logic on top of GPT-Neo's forward pass.
    """
    @classmethod
    def from_pretrained(
        cls,
        ...
        **kwargs,
        ...
        # Load base GPTNeoPreTrainedModel
        ...
        # GPT-Neo typically doesn't define a special pad_token_id. Use eos if needed.
    ...
    def initialize_user_model(
        ...
        if initialize_from_vocab:
            ...
            if most_common_tokens is None:
                vocab_size = self.transformer.wte.weight.size(0)  # gpt-neo wte
            ...
            init_value = self.transformer.wte(init_tokens).detach()
        ...
        print(f"Initialized {user_model_type} user model for GPT-Neo!")
    ...
    def _cat_user_embedding_to_input_sep(self, input_ids: torch.LongTensor, attention_mask: torch.LongTensor):
        ...
        For GPT-Neo, the embedding is transformer.wte(...).
        """
        ...
        for row_idx in range(batch_size):
            ...
            pad_embeds = self.transformer.wte(pad_tokens)
...
# ----------------------------------------------------------------------
# GPT-Neo–based reward model
# ----------------------------------------------------------------------
class UserGPTNeoForRewardModel(UserGPTNeoMixin, GPTNeoPreTrainedModel):
    """
    Minimal GPT-Neo-based Reward Model:
      - uses GPTNeoModel as the base
...
def main(script_args):
    ...
    # ----------------------------------------------------------
    # Initialize the GPT-Neo reward model
    # ----------------------------------------------------------
    if script_args.model_class != "gptneo":
        raise ValueError("This script is specialized for 'gptneo'. Set --model_class=gptneo.")
    ...
    peft_config = LoraConfig(
        ...
        target_modules=[
            # GPTNeo blocks typically have "attn", "mlp", etc. 
            # For example, "k_proj", "v_proj", "q_proj", "out_proj", 
            # you might need to adapt for GPTNeo naming:
            "attn.k_proj",
            "attn.v_proj",
            "attn.q_proj",
            "attn.out_proj",
    ...
    model_class = UserGPTNeoForRewardModel
    ...
    # GPT-Neo typically doesn't cache the same way as Llama; but for safety:
    ...
    print(f"Starting GPT-Neo Reward Model training with user_model={script_args.user_model} ...")
