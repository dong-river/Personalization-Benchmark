...
class PairwiseAutoAnnotator:
    ...
    def __init__(
        ...
        **decoding_kwargs,
        ...
        self.input_output_keys = self.input_keys + ["output_1","output_2"]
    ...
    def annotate_pairs(
        ...
        **decoding_kwargs :
        ...
        # print(df_to_annotate)
        # print('#'*30)
        # print(len(df_to_annotate))
        # print('#'*30)
    ...
    def _preprocess(self, to_annotate: ann_utils.AnyData) -> pd.DataFrame:
        ...
        df_to_annotate["chain_of_thoughts"] = np.nan
    ...
    def _merge_annotations(
        ...
        df_to_annotate = df_to_annotate.merge(
            df_partially_annotated[self.all_keys + ["preference","chain_of_thoughts"]],
            on=self.all_keys,
            how="left",
            suffixes=("_old", "_new"),
        )
        ...
        df_to_annotate["chain_of_thoughts"] = df_to_annotate["chain_of_thoughts_old"].fillna(
            df_to_annotate["chain_of_thoughts_new"]
        )
        df_to_annotate = df_to_annotate.drop(
            columns=["preference_old", "preference_new"]
        )
        df_to_annotate = df_to_annotate.drop(
            columns=["chain_of_thoughts_old", "chain_of_thoughts_new"]
        )
        return df_to_annotate
...
class SinglePairwiseAutoAnnotator:
    ...
    def __call__(self, df_to_annotate: pd.DataFrame, **decoding_kwargs) -> pd.DataFrame:
        ...
        while True:
            ...
            df_to_annotate["preference"],df_to_annotate["chain_of_thoughts"] = self.parse_completions(completions=completions)
            break_flg=True
            for x in df_to_annotate['preference']:
                if x == np.nan:
                    break_flg=False
            if break_flg:
                break
    ...
    def parse_completions(self, completions: list[str]) -> list[int]:
        ...
        for completion in completions:
            ...
            import re
            pattern4 = r'### Thought for example 4(.*?)### Result for example 4'
            # pattern5 = r'### Thought for Example 5(.*?)### Result for example 5'
            # pattern6 = r'### Thought for Example 6(.*?)### Result for example 6'
            # pattern7 = r'### Thought for Example 7(.*?)### Result for example 7'
            completion = re.sub(pattern4,"### Thought for example 4### Result for example 4",completion,flags=re.DOTALL)
            # completion = re.sub(pattern5,"### Thought for Example 5### Result for example 5",completion,flags=re.DOTALL)
            # completion = re.sub(pattern6,"### Thought for Example 6### Result for example 6",completion,flags=re.DOTALL)
            # completion = re.sub(pattern7,"### Thought for Example 7### Result for example 7",completion,flags=re.DOTALL)
            ...
            all_chain_of_thoughts.append(orig_completion)
        return all_preferences, all_chain_of_thoughts
    ...
    def postprocess(self, df_annotated: pd.DataFrame) -> pd.DataFrame:
        ...
        assert set(df_annotated["preference"].unique().tolist()) <= {1, 2, 3}
