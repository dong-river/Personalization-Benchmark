...
## Example usage: python ft_rm_general.py --train --eval --train_dataset_size 200000 --eval_data_size 1000 --data_type binarized_ultrafeedback --subset default --per_device_train_batch_size 16 --gradient_accumulation_steps 32 --learning_rate 0.0003 --num_train_epochs 1 --model_name google/gemma-2b-it --log_dir /home/yd358/rds/rds-semioticsteam-gLFG2ebmCDI/river_cache/models
## data_type should be binarized_ultrafeedback!!
## torchrun --nproc_per_node=4 --nnodes=1 --master_port=29522 ft_rm_general.py --train --eval --train_dataset_size 100000 --eval_data_size 1000 --data_type binarized_ultrafeedback --subset default --per_device_train_batch_size 16 --gradient_accumulation_steps 32 --learning_rate 0.0003 --num_train_epochs 1 --model_name google/gemma-2b-it --log_dir /home/yd358/rds/rds-semioticsteam-gLFG2ebmCDI/river_cache/models
## torchrun --nproc_per_node=4 --nnodes=1 --master_port=29522 ft_rm_general.py --train --eval --train_dataset_size 100000 --eval_data_size 1000 --data_type psoups --subset default --per_device_train_batch_size 16 --gradient_accumulation_steps 8 --learning_rate 0.0003 --num_train_epochs 1 --model_name google/gemma-2b-it --log_dir /home/yd358/rds/rds-semioticsteam-gLFG2ebmCDI/river_cache/models
...
class ScriptArguments:
    ...
    aggregation_method: str = field(
        default="concatenation", ## concatenation, majority_vote
        metadata={"help": "The aggregation method to use for the reward model."},
    )
    ...
    data_type: str = field(
        default="binarized_ultrafeedback",
...
if __name__ == "__main__":
    ...
    if script_args.train:
            ...
            if script_args.aggregation_method == 'majority_vote':
                df = train_dataset.to_pandas()
                voted_df = pd.DataFrame() 
                df['chosen_rejected'] = df.apply(lambda x: ' '.join(sorted([x['chosen_only'], x['rejected_only']])), axis=1)
                grouped = df.groupby("chosen_rejected")
                for name, group in grouped:
                    chosen_only = Counter(group['chosen_only'].tolist()).most_common(1)[0][0] ## get the majority chosen one
                    row = group[group['chosen_only'] == chosen_only][:1]
                    voted_df = pd.concat([voted_df, row])
                voted_df = voted_df.reset_index(drop=True)
                train_dataset = Dataset.from_pandas(voted_df) 
